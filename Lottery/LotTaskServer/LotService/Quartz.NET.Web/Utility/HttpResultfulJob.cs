    ...
    public class HttpResultfulJob : IJob
    {
        ...
        public async Task Execute(IJobExecutionContext context)
        {
            ...
            try
            {
                ...
                TaskInfo taskInfo = AppSetting.TaskInfo;
                if (taskInfo != null)
                {
                    string appId = taskInfo.AppId;
                    string secret = taskInfo.Secret;
                    string authorization = taskInfo.Authorization;
                    string randomNumber = DateTime.Now.ToString("yyyyMMddHHmmssfff");

                    var sign = GetSignStr(appId,secret,authorization,randomNumber);

                    header.Add("AppId", appId);
                    header.Add("Secret", secret);
                    header.Add("Authorization", authorization);
                    header.Add("RandomNumber", randomNumber);
        ...
        public async Task ExecuteOld(IJobExecutionContext context)
        {
            DateTime dateTime = DateTime.Now;
            TaskOptions taskOptions = context.GetTaskOptions();
            string httpMessage = "";
        ...
        private string GetSignStr(string appId, string secret, string Authorization, string randomNumber)
        {
