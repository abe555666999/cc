...
namespace Coldairarrow.Api.Controllers.Base_Manage
{
    ...
    public class TestInfoController : BaseApiController
    {
        ...
        /// <returns></returns>
        [HttpGet,AllowAnonymous]
        public async Task<IActionResult> GetTestPoPoFortunateData(string url)
        {
            var list = new List<object>();
            var str = await HttpHelper.GetStr(url);
            if (!str.IsNullOrEmpty())
            {
                ...
                var htmlNodeList = nodeInfo.SelectNodes("//div[@id='item1']//table//tr");
                 list = LotHtmlHelper.GetPOFortunateOpenResultData(htmlNodeList);
                if (list.Any())
                {
            ...
            return  await Task.FromResult(Content(result));
        }
