...
namespace Coldairarrow.Business.Dapper
{
	public interface ISqlDapper
	{
		...
		PageResult<T> GetPageList<T>(PageSelInput pageInput);
		...
		/// <param name="sqlBulkCopyOptions"></param>
		/// <returns></returns>
		int BulkInsert<T>(List<T> entities, string tableName = null, Expression<Func<T, object>> columns = null, SqlBulkCopyOptions? sqlBulkCopyOptions = null);
	}
